package com.kauel.kuploader2.utils

const val URL_LOGIN = "login"
const val URL_FILE = "processFile"
const val URL_TEST_FILE = "uploadTest"
const val URL_BASE = "https://alertaenap-testing.azurewebsites.net/api/"

const val ERROR_404 = "HTTP 404 Not Found"
const val ERROR_403 = "HTTP 403 Forbidden"
const val ERROR_LOGIN = "Email o password incorrect!"
const val ERROR_SERVER = "Error server!"

const val ADD_SERVER_RESULT_OK = 1
const val UPDATE_SERVER_RESULT_OK = 2

const val ACTION_START_SERVICE = "ACTION_START_SERVICE"
const val ACTION_STOP_SERVICE = "ACTION_STOP_SERVICE"

const val NOTIFICATION_CHANNEL_ID = "upload_channel"
const val NOTIFICATION_CHANNEL_NAME = "upload_channel_name"
const val NOTIFICATION_ID = 199

const val TITLE_INTENT_FOLDER = "Select directory"
const val CODE_INTENT_FOLDER = 111
const val FOLDER_HISTORY = "Historial imágenes subidas"
const val FOLDER_SUCCESS = "Success"
const val FOLDER_ERROR = "Error"

const val EMPTY_PATH = "Ruta sin imágenes!"
const val ERROR_PATH = "Error al seleccionar ruta!"

const val UPLOAD_START = "Subida iniciada!"
const val UPLOAD_STOP = "Subida detenida!"

const val NOTIFICATION_ERROR = "Error al subir imagenes"
const val NOTIFICATION_UPLOAD_FINISHED = "Subida terminada!"
const val NOTIFICATION_TITLE = "Subida de imágenes"
const val NOTIFICATION_UPLOAD_STOP = "Subida detenida!"